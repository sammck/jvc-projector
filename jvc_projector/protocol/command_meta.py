...
For more recent projectors, further information here:
...
class JvcModel:
    ...
    sddp_name: str
    """The name provided in the "Model" field by the projector in its SDDP responses"""

    model_status_payload: bytes = b''
    """The adanced response payload returned by the projector in response to a
    ...
    def __repr__(self) -> str:
        return f"JvcModel('{self.default_name}')"
...
"""A list of lists of projector models that are known at the time this metadata was
...
models: Dict[str, JvcModel] = {}
"""A dictionary of projector models, keyed by model name."""
...
for _model_names in _known_models:
    ...
    _model = JvcModel(_expanded_model_names)
    for _model_name in _expanded_model_names:
        if _model_name in models:
            raise ValueError(f"Model name associated with multiple JvcModels: '{_model_name}'")
...
"""Response payloads for model_status.query command, and a list of projector
   model names they correspond to. Alternative equivalent model names will
   be automatically effectively added to each list when normalization to JvcModel is done.
   If more than one JvCModel is associated with a given payload, the first one in the list
...
model_status_list_map: Dict[bytes, Tuple[Set[JvcModel], JvcModel]] = {}
"""Response payloads for model_status.query command, and the set of projector
    models they correspond to, along with the default model associated with the payload."""

def _fix_model_status_list_map() -> None:
    seen_models: Set[JvcModel] = set()
    for model_status_payload, model_names in _model_status_list_name_map.items():
        group_models: Set[JvcModel] = set()
        default_model: Optional[JvcModel] = None
        for model_name in model_names:
            if not model_name in models:
                raise ValueError(f"Model name {model_name} is not associated with any JvcModel")
...
class CommandGroupMeta:
    ...
    """For command groups that apply to a subset to projector models, a slash-delimited
       list of projector models that support this command group. Items in the list do not
    ...
    models: Optional[Set[JvcModel]]
    """For command groups that apply to a subset to projector models, the set of
       projector models that support this command group.
    ...
    def parse_models_str(self, models_str: Optional[str]) -> Optional[Set[JvcModel]]:
        if models_str is None:
            return None
        parsed_models: Set[JvcModel] = set()
...
class CommandMeta:
    ...
    models: Optional[Set[JvcModel]] = None
...
_group_metas: List[CommandGroupMeta] = [
    ...
    _G("remote_control", b'\x52\x43\x37\x33', commands=[
        ...
        _C("colour_management_next", b'\x38\x39', "Colour Management - Cycles through: Off/ Custom 1/Custom 2/Custom 3 (X7/X9/X70/X90/RS50/60/55/65)",
           models_str="X7/X9/X70/X90/RS50/60/55/65"),
        # note: the following command is documented as being supported by "X79", but that projector
