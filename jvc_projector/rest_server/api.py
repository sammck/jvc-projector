...
async def root(request: Request):
    client: JvcProjectorClient = request.app.state.jvc_client
    await client.cmd_null()
    return { "message": f"Hello World! Serving projector at {client}" }

@router.get("/version")
async def version():
    """Returns the jvc-projector package version"""
    return { "version": pkg_version }

@router.get("/config")
async def config_data(request: Request) -> Dict[str, Any]:
    """Returns the current configuration of jvc-projector.
...
async def execute_one_command(client: JvcProjectorClient, cmd_name: str) -> JsonableDict:
...
async def execute(
    ...
    """Executes a single projector command by name and returns the result."""
...
async def ping(
    ...
    """Returns the health status of the API server and the projector."""
    client: JvcProjectorClient = request.app.state.jvc_client
    ...
    try:
        ...
        result["projector_status"] = "ERROR"
        proj_error = full_class_name(exc)
        result["projector_error"] = proj_error
        msg = str(exc)
        if msg == "":
            msg = proj_error
        result["projector_error_message"] = msg
    else:
        result["projector_status"] = "OK"
    return result
...
async def power_on(
    ...
    """Turns the projector on if necessary, and waits for the power on sequence to complete."""
...
async def power_off(
    ...
    """Turns the projector off if necessary, and waits for the power off sequence to complete."""
